...
import { departments, mockUniversities } from '@/data/mockData';
import { Leaf, Loader2, Eye, EyeOff, CheckCircle2, Circle, Building2 } from 'lucide-react';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[95vh] overflow-y-auto bg-background">
        <DialogHeader className="items-center">
          ...
          <DialogTitle className="text-2xl font-bold tracking-tight">RouteMate</DialogTitle>
          <DialogDescription>Pakistan's Student Carpooling Network</DialogDescription>
        </DialogHeader>
          ...
          <TabsContent value="signin" className="space-y-4 pt-4">
            <form onSubmit={signInForm.handleSubmit(handleSignIn)} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="si-email" placeholder="student@nutech.edu.pk" {...signInForm.register('email')} />
              </div>
              <div className="space-y-2">
                <Label htmlFor="si-password">Password</Label>
                <div className="relative">
                  <Input id="si-password" type={showPassword ? 'text' : 'password'} {...signInForm.register('password')} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-2.5 text-muted-foreground hover:text-primary">
          ...
          <TabsContent value="signup" className="space-y-4 pt-2">
            <form onSubmit={signUpForm.handleSubmit(handleSignUp)} className="space-y-3">
              <div className="space-y-1">
                ...
                <Input placeholder="Ahmed Ali" {...signUpForm.register('name')} />
              </div>

              <div className="space-y-1">
                <Label>University Email</Label>
                <Input placeholder="name@nutech.edu.pk" {...signUpForm.register('email')} />
                ...
                {detectedUni ? (
                  <div className="flex items-center mt-2 p-2 bg-primary/5 border border-primary/20 rounded-md">
                    ...
                    <span className="text-xs font-medium text-primary uppercase">Registering for: {detectedUni.name}</span>
                  </div>
                ) : emailValue?.includes('@') && (
                  <div className="mt-2 p-3 bg-amber-50 border border-amber-200 rounded-md">
                    ...
                    {!showRequestForm ? (
                      ...
                      <div className="mt-2 space-y-2">
                        ...
                        >
                          {isLoading ? <Loader2 className="animate-spin h-3 w-3" /> : 'Submit Request'}
                        </Button>
                      </div>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <Label>Gender</Label>
                  <Select onValueChange={(v) => signUpForm.setValue('gender', v as any)}>
                ...
                <div className="space-y-1">
                  ...
                  <Select onValueChange={(v) => signUpForm.setValue('department', v)}>
                    <SelectTrigger><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <Input type={showPassword ? 'text' : 'password'} {...signUpForm.register('password')} />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-2.5 text-muted-foreground hover:text-primary">
                    {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
                  </button>
                </div>
                ...
                <div className="grid grid-cols-2 gap-x-2 gap-y-1 pt-2 border-t mt-2">
                  {checks.map((c, i) => (
                    <div key={i} className={`flex items-center text-[10px] ${c.met ? 'text-green-600' : 'text-muted-foreground'}`}>
                      {c.met ? <CheckCircle2 size={12} className="mr-1" /> : <Circle size={12} className="mr-1" />}
                      {c.label}
                    </div>
                  ))}
                </div>
              ...
              <Button type="submit" className="w-full" disabled={isLoading || !passwordRegex.test(pwd)}>
